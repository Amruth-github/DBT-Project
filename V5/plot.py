...
consumer_stream = KafkaConsumer('aggregate_count', group_id='plot', bootstrap_servers=["localhost:9092"],auto_offset_reset = 'latest', value_deserializer=lambda x: json.loads(x))
consumer_batch = KafkaConsumer('aggregate_count_batch', bootstrap_servers=["localhost:9092"],auto_offset_reset = 'latest', value_deserializer=lambda x: json.loads(x))
# Data to plot
